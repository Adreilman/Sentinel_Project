...
try:
    ...
    st.error(f"Could not load Sentinel dashboard data: {exc}")
    st.stop()
...
st.sidebar.caption(
    "Dashboard dataset: compact deployment subset"
)
...
if page == "Overview":
    ...
    detected_ips = int(
        final_df["detected_in_any_stage"].sum()
    )

    stage1_ips = int(
        final_df["stage1_detected"].sum()
    )

    stage2_ips = int(
        final_df["stage2_detected"].sum()
    )

    stage3_ips = int(
        final_df["stage3_detected"].sum()
    )
    ...
    with c3:
        st.metric(
            "All-Stage Detections",
            f"{all_stage_ips:,}",
        )

    with c4:
        st.metric(
            "Reference Suspicious Detected",
            "15 / 15",
        )
    ...
    # --------------------------------------------------------
    st.subheader(
        "Highly Corroborated Detections"
    )
    ...
    if not corroborated.empty:
        ...
        columns = [
            ...
            if column in corroborated.columns
        ]

        corroborated = corroborated.sort_values(
            [
                "detection_stage_count",
                "mean_score",
            ],
...
# ============================================================
    ...
    st.markdown(
        '<div class="sentinel-subtitle">'
        "Inspect behavioral activity for an individual IP"
        "</div>",
        unsafe_allow_html=True,
    )
    ...
    # Final IP-level data
    ip_final = final_df[
        final_df["remote_host"].astype(str)
        == selected_ip
    ]
    ...
    if ip_final.empty:
        ...
        st.warning(
            "This IP is not present in the dashboard dataset."
        )
    ...
    # Minute-level feature data
    ip_features = features_df[
        features_df["remote_host"].astype(str)
        == selected_ip
    ].copy()
    ...
    with s3:
        ...
        st.metric(
            ...
            (
                ...
                else f"{float(min_score):.5f}"
            ),
        )

    with s4:

        mean_score = ip_final.get(
            "mean_score",
            None,
        )
    ...
    if ip_features.empty:
        ...
        st.info(
            "No retained minute-level feature rows "
            "are available for this IP."
        )
    ...
    if "minute_bucket" in ip_features.columns:
        ...
        ip_features = ip_features.sort_values(
            "minute_bucket"
        )
...
# ============================================================
    ...
    if filtered.empty:
        ...
        st.caption(
            "Displaying up to 1,000 matching IPs."
        )
...
# ============================================================
    ...
    st.info(
        "Reference set: 30 manually labeled IPs — "
        "15 suspicious and 15 legitimate. "
